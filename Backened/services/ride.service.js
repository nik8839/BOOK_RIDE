  ...
  const fare = {
    auto: Math.round(
      baseFare.auto +
        ...
        (distanceTime.time / 60) * perMinuteRate.auto
    ),
    car: Math.round(
      baseFare.car +
        (distanceTime.distance / 1000) * perKmRate.car +
        (distanceTime.time / 60) * perMinuteRate.car
    ),
    moto: Math.round(
      baseFare.moto +
        (distanceTime.distance / 1000) * perKmRate.moto +
        (distanceTime.time / 60) * perMinuteRate.moto
    ),
  };
...
module.exports.endRide = async ({ rideId }) => {
  ...
  //console.log("hello in ride service")
  const ride = await rideModel
    .findOne({
      _id: rideId,
      // captain: captain._id,
